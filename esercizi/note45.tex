...
In questo caso la derivazione di $\Gamma$ $\vdash$ $M$ : $T$ si riesce ad ottenere usando la regaola $IfThenElse$, tuttavia questo impone che nel contesto ci siano questi giudizi:
\begin{itemize}
...
\item $fn$ $x$:$T_1.fn$ $y$:$T_2.if$ $y$ then $x$ else true  \\
		...
		\begin{prooftree}
			...
			\AxiomC{A}
			\UnaryInfC{$f$:$T$,$x$:$T^{\prime}$ $\vdash$ $f$:$^{\prime\prime\prime\prime} \rightarrow T^{\prime\prime}$}
			\AxiomC{B}
			\UnaryInfC{$f$:$T$,$x$:$T^{\prime}$ $\vdash$ $ if true$ then $x$ else $f$ $x$ : $T_3$}
			\LeftLabel{\textsc{(APP)}}
		...
		Il risultato finale \`e dato dal fatto che $T = T_1 = T_" = T_3 = T_4$ quindi $(T \rightarrow T) \rightarrow (T \rightarrow T) $
		\vspace{1.5cm}

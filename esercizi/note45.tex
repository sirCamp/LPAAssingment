...
Supponimao di avere $M^{\prime}$ $\equiv$ $A$ : $T$ e $M$ $\equiv$ $if$ $true$ $then$ $A$ $else$ $B$, ovvero il caso in cui $M$ $\rightarrow$ $M^{\prime}$ per la regola (If-True).\\
In questo caso la derivazione di $\Gamma$ $\vdash$ $M$ : $T$ ri riesce ad ottenere usando la regaola $IF_THEN_ELSE$, tuttavia questo impone che nel contesto ci siano questi giudizi:
...
<<<<<<< Updated upstream
		Il risultato finale \`e dato dal fatto che $T = T^{\prime} = T^{\prime\prime} = T^{\prime\prime\prime} = T^{\prime\prime\prime\prime}$ quindi $(T \rightarrow T) \rightarrow (T \rightarrow T) $
=======

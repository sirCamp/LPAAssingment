...
Sulla base del fatto che i variant-type sono simili ai record, \`e possibile ipotizzare un subtyping basato sulle reogle SUB-WITH e SUB-DEPTH. I variant-type, infatti, hanno la stessa struttura, la differenza risiede nel fatto che i campi di quest'ultimi vengono acceduti tramite \textit{pattern matching} e non tramite etichette. La differenza principale sta nel fatto che un variant type con meno tipi \`e pi\`u informativo di uno che ne ha di pi\`u. Considerando il pattern matching, infatti, un variant type con meno tipi pu\`o essere usato in ogni caso in cui il pattern matching gestisca pi\`u casi. Non \`e vero il contrario. \\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{enumerate}
			...
			\scalebox{.90}{
		     $\{\ell_i : v_i\ ^{i \in 1 \dots x-1}, \ell_x : M_x, \ell_j : M_j\ ^{j \in x+1 \dots m}\}$ 	$\longrightarrow \{\ell_i : v_i\ ^{i \in 1 \dots x-1}, \ell_x : M'_x, \ell_j : M_j\ ^{j \in x+1 \dots m}\}$ }
...
%	\`e leggermente differente a quella mostrata nell'esercizio 6.3. Mentre la regola \myrule{Subsumption} \`e ``inedita":
	...
	\begin{itemize}
		...
		\parbox{1cm}{
		\begin{prooftree} 
		\AxiomC{$\emptyset \vdash_{k} M : S$}
		\AxiomC{$ S <: T $}
		\LeftLabel{\myrule{Subsumption}}
		\BinaryInfC{$\emptyset \vdash_{k+1} M : T$  } 
		\end{prooftree}
	}}
...
$ (fn$ x:$T.x)\{l_1 = 1, l_2 = 2 \} $. Questo termine ha tipo T = $\{\ l_1: Nat\}$.
Infatti, possiamo passare un record con tipo S<:T dove \`e richiesto un record di tipo T.

A questo punto, si nota che il risultato dell'applicazione sar\`a $\{l_1 = 1, l_2 = 2 \} $ dove il suo tipo \`e S. 

...
\begin{enumerate}[label=\alph*), leftmargin=*]
	...
	\item f:Bool$\rightarrow$ Bool $\vdash\:\fn$ x:Bool.f(if x then false else x) :Bool$\rightarrow$Bool 
		...
		\begin{prooftree}  
			...
			\LeftLabel{\textsc{(FUN)}}
			\UnaryInfC{$\Gamma$ $\vdash\:\fn$ x:Bool.f: Bool $\rightarrow$ Bool 
			$\rightarrow$ Bool} 
			...
			\UnaryInfC{$\Gamma$ $\vdash$ false :Bool}
			\LeftLabel{\textsc{(IF-THEN-ELSE)}}
			\TrinaryInfC{$\Gamma$ $\vdash$ (if x then false else x):Bool}
			\LeftLabel{\textsc{(APP)}}
		...
		Quindi il tipo \`e $Bool$ $\rightarrow$ $(Bool \rightarrow Bool)$.	
	...
	\item f:Bool$\rightarrow$ Bool $\vdash\:\fn$ x:Bool.f(if x then false else x) :Bool$\rightarrow$Bool 
		...
		\begin{prooftree}  
			...
			\LeftLabel{\textsc{(APP)}}
			\BinaryInfC{$f(if$ $x$ $then$ $false$ $else$ $x)$ :$Bool$ $\rightarrow$ $Bool$}
			
			\LeftLabel{\textsc{(FUN)}}
			\BinaryInfC{$\Gamma\:\vdash\:\fn$ $x:Bool.f(if$ $x$ $then$ $false$ $else$ $x)$ :$Bool$ $\rightarrow$ $Bool$}
		\end{prooftree}}
		} \\
